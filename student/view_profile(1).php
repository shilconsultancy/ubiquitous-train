...
$announcements = [];
$sql_announcements = "SELECT title, content, published_date FROM notices WHERE status = 'Published' ORDER BY published_date DESC LIMIT 3";
...
<body class="bg-gray-50 text-gray-800 flex flex-col min-h-screen">
        ...
        <main class="flex-1 p-4 sm:p-6 pb-24 md:pb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-1 bg-white rounded-xl shadow-custom p-6 text-center fade-in">
                    ...
                    <p class="text-lg font-bold mt-2">Role: <span class="text-primary"><?= htmlspecialchars(ucfirst($user_data['role'])); ?></span></p>
            ...
            <div class="mt-8 fade-in" style="animation-delay: 0.3s;">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Latest Notices</h2>
                <?php if (!empty($announcements)): ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <?php foreach ($announcements as $announcement): ?>
                            <div class="bg-white rounded-xl shadow-custom p-6 flex flex-col hover:shadow-lg transition-shadow duration-300">
                                <div class="flex items-start space-x-3 mb-3">
                                    <i class="fas fa-bullhorn text-theme-red mt-1 text-xl"></i>
                                    <h4 class="font-bold text-lg text-gray-800"><?php echo htmlspecialchars($announcement['title']); ?></h4>
                                </div>
                                <p class="text-gray-600 text-sm flex-grow"><?php echo nl2br(htmlspecialchars($announcement['content'])); ?></p>
                                <p class="text-xs text-gray-400 mt-4 pt-4 border-t border-gray-100 text-right"><?php echo date('d M Y', strtotime($announcement['published_date'])); ?></p>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
                    <div class="bg-white rounded-xl shadow-custom p-8 text-center text-gray-500">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const mobileMoreBtn = document.getElementById('mobile-menu-button');
            const mobileMoreMenu = document.getElementById('mobile-menu');

            if(mobileMoreBtn){
                mobileMoreBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    mobileMoreMenu.classList.remove('-translate-x-full');
                });
            }
            ...
            if (mobileMenuCloseBtn) {
                mobileMenuCloseBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    mobileMoreMenu.classList.add('-translate-x-full');
                });
            }

            // Close mobile menu if clicked outside
            document.addEventListener('click', (e) => {
                if (mobileMoreMenu && !mobileMoreMenu.classList.contains('-translate-x-full') && !mobileMoreBtn.contains(e.target) && !mobileMoreMenu.contains(e.target)) {
                    mobileMoreMenu.classList.add('-translate-x-full'); 
                }
            });
        });
