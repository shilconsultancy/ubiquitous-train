...
$allowed_statuses = ['Paid', 'Partially Paid', 'Pending', 'Overdue', 'Cancelled'];
$filter_status = isset($_GET['status']) && in_array($_GET['status'], $allowed_statuses) ? $_GET['status'] : '';

$sort_columns = ['issue_date', 'due_date', 'total_amount', 'username', 'invoice_number'];
...
<head>
    ...
    <style>
        ...
        @media (max-width: 767px) {
            ...
            .flex-col.md\:flex-row.gap-4 { flex-direction: column; align-items: stretch; }
            .w-full.md\:w-auto { width: 100%; }
            .w-full.sm\:w-auto.mt-4.sm\:mt-0 { width: 100%; }
            .flex.items-center.space-x-3 { justify-content: flex-start; }
        }
    ...
    <div class="flex flex-1">
        ...
        <main class="flex-1 p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                ...
                <a href="add_invoice.php" class="w-full sm:w-auto mt-4 sm:mt-0 bg-primary hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center justify-center">
                            ...
                            <?php if (!empty($invoices_data)): foreach ($invoices_data as $invoice): ?>
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td data-label="Status" class="p-4">
                                        <span class="text-xs font-medium px-2 py-1 rounded-full <?= match ($invoice['status']) { 'Paid' => 'bg-green-100 text-green-800', 'Partially Paid' => 'bg-blue-100 text-blue-800', 'Pending' => 'bg-yellow-100 text-yellow-800', 'Overdue' => 'bg-red-100 text-red-800', 'Cancelled' => 'bg-gray-100 text-gray-800', default => 'bg-gray-100 text-gray-800' }; ?>"><?= htmlspecialchars($invoice['status']); ?></span>
                                    </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (userMenuButton && userMenu) {
                ...
                document.addEventListener('keydown', function(event) { if (event.key === 'Escape') { userMenu.classList.add('hidden'); } });
            }
            ...
            if (mobileMoreBtn && mobileMoreMenu) {
                ...
                document.addEventListener('click', (e) => { if (!mobileMoreBtn.contains(e.target) && !mobileMoreMenu.contains(e.target)) { mobileMoreMenu.classList.add('hidden'); } });
            }
        });
    </script>
</body>
