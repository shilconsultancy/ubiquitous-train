    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($_FILES['profile_image']['type'], $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $destination)) {
                $default_avatar = 'assets/images/default_avatar.png'; // This is relative to admin/
                // Check if the old image is not the default one and exists before unlinking
                if (!empty($profile_image_path) && $profile_image_path !== $default_avatar && file_exists('../' . $profile_image_path)) {
                    unlink('../' . $profile_image_path);
                }
                // **FIX:** Store a root-relative path in the database
...
<head>
    ...
    <style>
        /* CSS from previous version remains the same, it is well-structured */
        ...
        @media (max-width: 767px) {
            ...
            .flex.justify-between.items-center.mb-8 > * { margin-top: 1rem; }
            .flex.justify-between.items-center.mb-8 > *:first-child { margin-top: 0; }
            .grid.grid-cols-1.md\:grid-cols-2.gap-6 { grid-template-columns: 1fr; }
            .mt-2.flex.items-center.space-x-4 { flex-direction: column; align-items: flex-start; space-x: 0; space-y: 1rem; }
            .flex.items-center > label[for="profile_image"] { margin-left: 0 !important; }
            .sr-only + span { margin-left: 0 !important; display: block; width: 100%; }
                    ...
                    <form action="edit_student.php?id=<?= htmlspecialchars($student_id); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                        <div>
                            ...
                            <div class="mt-2 flex items-center space-x-4">
                                <?php
                                    // **FIX:** Path logic updated to correctly find images from the root
